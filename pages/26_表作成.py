...
st.title("🧾 表作成")
st.caption(
    "1行目から=ヘッダー、ヘッダーの後=データです。Excelからそのまま貼り付けてください。"
)
st.caption(
    "ヘッダーがない場合はヘッダー行数を 0 にしてください。行見出しがある場合はヘッダー列数も指定できます。"
    "セルに「<同上>」と書くと上のセルと縦結合、「<同左>」と書くと左のセルと横結合されます。また「<改行>」はセル内改行として扱われます。"
)
