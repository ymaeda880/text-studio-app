...
# ・同一値の縦/横連続セルを自動結合（ヘッダー除外オプションあり）
# ・結合セルは基準セル（左上）だけ表示
...
def _apply_table_borders_robust(
    ...
    *,
    ...
    """
    結合後の「実セル」に対して罫線を設定する堅牢版。
    内側線は “両側” に当てる（下=top/bottom、右=left/right）ことで消失を防ぐ。
    """
    R, C = len(spans), len(spans[0])

    def is_anchor(r, c):
        return not spans[r][c]["skip"]

    # 内側横線
    if inner_h:
        for r in range(R):
            for c in range(C):
                if not is_anchor(r, c):
                    continue
                rs = spans[r][c]["rowspan"]
                br = r + rs - 1
                if br < R - 1:
                    below_cell = table.cell(br + 1, c)
                    here_cell  = table.cell(r, c)
                    _set_cell_border(here_cell,  bottom=("single", sz_inner, color))
                    _set_cell_border(below_cell, top   =("single", sz_inner, color))
    ...
    if inner_v:
        for r in range(R):
            for c in range(C):
                if not is_anchor(r, c):
                    continue
                cs = spans[r][c]["colspan"]
                rc = c + cs - 1
                if rc < C - 1:
                    right_cell = table.cell(r, rc + 1)
                    here_cell  = table.cell(r, c)
                    _set_cell_border(here_cell,  right=("single", sz_inner, color))
                    _set_cell_border(right_cell, left =("single", sz_inner, color))
...
def _compute_spans(
    ...
    strict_rect: bool = True,   # 横結合は矩形制約を満たす列だけに限定
):
    """
    rowspan/colspan と skip 情報を返す。
    1) 縦方向の連続同一値をまとめ（上端セルに rowspan>1）
    2) 横方向の連続同一値をまとめ（左端セルに colspan>1）
       - strict_rect=True のとき、横にまたぐ全列で同じ rowspan を持つ場合にのみ横結合
         （＝Wordの矩形セル結合要件を満たす）
    3) 空文字・None は結合しない
    """
    if not rows:
    ...
    if merge_v:
        for c in range(C):
            ...
            while r < R:
                ...
                while end < R and (not spans[end][c]["skip"]) and _norm(rows[end][c]) == val:
                    end += 1
                run_len = end - r
                if run_len > 1:
                    spans[r][c]["rowspan"] = run_len
                    for rr in range(r + 1, end):
                        spans[rr][c]["skip"] = True
                r = end
    ...
    if merge_h:
        for r in range(start_r, R):
            ...
            while c < C:
                ...
                base_rowspan = spans[r][c]["rowspan"]
                end = c + 1
                run_cols = [c]
                ...
                while end < C:
                    if spans[r][end]["skip"]:
                        break
                    if _norm(rows[r][end]) != val:
                        break
                    # 矩形制約：横に広げる全列で「同じrowspan」を要求
                    if strict_rect and spans[r][end]["rowspan"] != base_rowspan:
                        break
                    run_cols.append(end)
                    end += 1

                if len(run_cols) > 1:
                    spans[r][c]["colspan"] = len(run_cols)
                    for cc in run_cols[1:]:
                        spans[r][cc]["skip"] = True
                c = end
    return spans
...
def _merge_docx_by_spans(
    ...
    for r in range(R):
        for c in range(C):
            ...
            cs = info["colspan"]
            if rs > 1 or cs > 1:
                tgt = table.cell(r + rs - 1, c + cs - 1)
                table.cell(r, c).merge(tgt)
...
def _apply_table_inner_borders_after_merge(table, spans, vertical=True, horizontal=True, val="single", sz=6, color="000000"):
    """結合後、見えるセルだけに内側罫線を当てる（内部に線を残さない安全版）"""
    R, C = len(spans), len(spans[0])
    if horizontal:
        for r in range(R - 1):
            for c in range(C):
...
with st.sidebar:
    ...
    st.markdown("### 2) 同一値セルの自動結合")
    merge_v = st.checkbox("縦方向に同一値を結合", value=True)
    merge_h = st.checkbox("横方向に同一値を結合", value=True)
    ignore_header = st.checkbox("ヘッダー行は結合対象に含めない", value=True)
...
if make_btn:
    ...
    # spans 計算（縦→横、ヘッダー除外オプションあり）
    spans = _compute_spans(rows, merge_v=merge_v, merge_h=merge_h, ignore_header=ignore_header)

    # 実セル結合＋文字/背景スタイル（「基準セルだけ表示」版）
