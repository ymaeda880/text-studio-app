...
st.title("📊 棒グラフ作成")
st.caption("1行目=タイトル、2行目=ヘッダー、3行目以降=データ（※『タイトルなし』も選べます）。Excelからそのまま貼り付けてください。")
...
def mini_toggle(label: str, key: str):
    """エクスパンダの直前に置く小さめトグル（チェックボックス）"""
    # mini-toggle-row で囲むことで、上の CSS が効く
    st.markdown('<div class="mini-toggle-row">', unsafe_allow_html=True)
    st.checkbox(label, key=key)   # ラベル付きの checkbox にする
...
mode = st.session_state.get("k_bar_mode", "自動")
manual_factor = float(st.session_state.get("m_manual_bar_width", 0.60))  # 0.10〜1.00
...
# st.write("🔍 デバッグ: bar_width =", bar_width)
# st.write("🔍 デバッグ: mode =", mode)
