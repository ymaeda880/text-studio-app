...
def _apply_table_borders_robust(
    ...
    *,
    ...
    if inner_h:
        for r in range(R - 1):
            for c in range(C):
                ...
                # 同じ _tc なら1セル → 内部なので線を引かない
                if up._tc is down._tc:
                    continue
        ...
        for r in range(R):
            for c in range(C - 1):
                ...
                if left._tc is right._tc:
                    print(f"[SKIP] r={r}, c={c} は結合セル内部（縦線なし）")
                    continue
    ...
    # -----------------------------
    # 3) 外周だけ外枠を入れる（spans を使って「見えているセル」を選ぶ）
    # -----------------------------
    # -----------------------------
    # 3) 外周だけ外枠を入れる（spans 対応 / 結合セル完全対応）
    # -----------------------------
    if outer and spans:
        RR = len(spans)
        CC = len(spans[0])

        # visible = 実際に画面に見えているセル（アンカーセル）
        def visible(r: int, c: int) -> bool:
            return not spans[r][c]["skip"]

        # -----------------------------
        # 1) 上端（top）
        #    各列 c について、一番上の visible セルに top 枠
        # -----------------------------
        for c in range(CC):
            for r in range(RR):
                if visible(r, c):
        ...
        # -----------------------------
        # 2) 下端（bottom）
        #    結合があっても table.cell(RR-1, c) は
        #    最下端の実セル（アンカー）を返すためそのまま bottom を付けてよい
        # -----------------------------
        # 下端：各列ごとに “一番下に見えているセル” に bottom を引く
        for c in range(CC):
            for r in range(RR - 1, -1, -1):
                if visible(r, c):  # skip=False のセル
                    cell = table.cell(r, c)
        ...
        # 3) 左端（left）
        #    ＜重要＞
        #    左端は結合セル（rowspan）があると skip=True の行で線が抜ける。
        #    よって visible を使わず、結合チェーンのアンカーに対して left を付ける。
        # -----------------------------
        for r in range(RR):
            # (r,0) が skip=True ならアンカーまで遡る
            anchor_r = r
            while anchor_r > 0 and spans[anchor_r][0]["skip"]:
                anchor_r -= 1

            cell = table.cell(anchor_r, 0)
            _set_cell_border(cell, left=("single", sz_outer, color))

        # -----------------------------
        # 4) 右端（right）
        #    左端と同じく、結合セルの途中行では skip=True になるため
        #    結合チェーンのアンカーに対して right を付ける
        # -----------------------------
        for r in range(RR):
            anchor_r = r
            while anchor_r > 0 and spans[anchor_r][CC - 1]["skip"]:
                anchor_r -= 1

            cell = table.cell(anchor_r, CC - 1)
...
def _merge_docx_by_spans(
    ...
    """
    結合セルでは基準セル（左上/上端・左端アンカー）の文字だけを表示。
    1) まず全セルを空にする
    2) 結合を実行
    3) 基準セルのみテキストを書き込む
    4) 背景とフォントを適用
