...
# ------------------------------------------------------------
# Inboxで読み込んだテキストを「入力ソース」として復元（rerun対策）
# ------------------------------------------------------------
if (not src_text) and st.session_state.get(K_INBOX_LOADED_TEXT, "").strip():
    src_text = st.session_state[K_INBOX_LOADED_TEXT].strip()
    if not used_file_name:
        used_file_name = st.session_state.get(K_INBOX_LOADED_NAME, "") or "inbox_text.txt"
...
with tab_inbox:
    ...
    if not sub:
        ...
        if total <= 0 or not rows:
            ...
            with cbtn1:
                load_inbox_clicked = st.button("📥 Inboxテキストを読み込む", key="proof_load_from_inbox")
            with cbtn2:
                st.caption("※ 押すと本文にセットされ、この後『① 解析』を押すと解析します。")

            if load_inbox_clicked:
                if not selected_item_id:
                    st.warning("先にInboxのテキストを選択してください。")
                    st.stop()
                ...
                try:
                    ...
                    # ✅ session_state に保持（rerunしても残る）
                    st.session_state[K_INBOX_LOADED_TEXT] = txt.strip()
                    st.session_state[K_INBOX_LOADED_NAME] = str(picked.get("original_name") or "inbox_text.txt")

                    st.success("Inbox テキストを読み込みました。下の『① 解析（Inbox）』を押すと解析します。")
            ...
            _loaded = st.session_state.get(K_INBOX_LOADED_TEXT, "").strip()

            analyze_inbox_clicked = st.button(
                "① 解析（Inbox）",
                type="primary",
                disabled=not bool(_loaded),
                key="btn_analyze_inbox",
            )

            if analyze_inbox_clicked:
                st.session_state["proof_do_analyze_inbox"] = True
...
if src_text:
    ...
    # ------------------------------------------------------------
    # 解析トリガー
    # - file/paste は従来どおり locals()
    # - Inbox は session_state のワンショットフラグで起動
    # ------------------------------------------------------------
    want_analyze = bool(
        locals().get("do_analyze_file")
        or locals().get("do_analyze_paste")
        or st.session_state.pop("proof_do_analyze_inbox", False)
    )
