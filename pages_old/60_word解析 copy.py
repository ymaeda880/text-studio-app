...
# 中間テキストの構築
# =========================
def build_intermediate_text(doc: Document, base_chapter: int) -> Tuple[str, Dict[str, int]]:
    """
    Document 全体を走査して、中間テキストを 1 つ組み立てる。

    - 見出し:  === HEADING[3-1-2] タイトル ===
    - 本文:    そのままのテキスト行
    - 表:      表番号＋タイトルの行の下に JSON を埋め込む
    - 図:      図キャプション＋画像ファイル名
    ...
    heading_counters = [0, 0, 0, 0]  # 見出しレベル 1〜4 用
    ...
    for block_idx, block in enumerate(iter_block_items(doc), start=1):
        ...
        if isinstance(block, Paragraph):
            ...
            if cat == "table_caption":
                # 表キャプションは直後の Table とセットで処理したいので保存だけ
            ...
            prev_block = block
            # 表キャプションの後に空段落が続く場合もあるので pending はクリア
            pending_table_caption = None
            ...
            if cat == "table":
                ...
                # 表ブロック前に空行
                lines.append("")
                header_line = f"=== TABLE {tbl_num} {tbl_title} ===".rstrip()
...
with st.status("解析中です…", expanded=False) as status:
    intermediate_text, stats = build_intermediate_text(src_doc, base_chapter=int(base_chapter))
...
txt_name = f"{base_name}_intermediate.txt"
