...
# ─────────────────────────────────────────────
# 福島（fukushima）固定ヘッダー読み込みユーティリティ
# 3行目（index=2）がヘッダー、4行目からデータ
# ─────────────────────────────────────────────

# ─────────────────────────────────────────────
# 列名フラット化（MultiIndex → 単層）
# ─────────────────────────────────────────────
def _flatten_columns(df: pd.DataFrame) -> pd.DataFrame:
    g = df.copy()
    if isinstance(g.columns, pd.MultiIndex):
        cols = [
            " ".join([_clean_header_cell(c) for c in tup if (c is not None and str(c) != "nan")]).strip()
            for tup in g.columns.to_list()
        ]
    else:
        cols = [_clean_header_cell(c) for c in g.columns]
...
# ヘッダーっぽい語（検知用）
HEADER_CAND_WORDS = {
    "和名", "学名", "生物群名", "分類", "科名", "科", "目名", "属",
    "ふくしまRL2024カテゴリー", "ふくしまRL2022カテゴリー", "カテゴリー", "備考"
}

def _maybe_promote_first_row_to_header(df: pd.DataFrame) -> tuple[pd.DataFrame, int]:
    cols = list(map(str, df.columns))
    unnamed_ratio = sum(c.startswith("Unnamed") or c == "" for c in cols) / max(1, len(cols))

    first = df.iloc[0].astype(str).map(_clean_header_cell)
    HEADER_CAND_WORDS = {
        "和名", "学名", "生物群名", "分類", "科名", "科", "目名",
        "ふくしまRL2021 カテゴリー(旧)", "ふくしまRL2022 カテゴリー", "ふくしまRL2024 カテゴリー", "カテゴリー", "備考"
    }
    hits = sum(v in HEADER_CAND_WORDS for v in first.values)

    if unnamed_ratio >= 0.3 and hits >= 2:
        # 0行目を列名として採用
        new_cols_raw = [(_clean_header_cell(x) or "") for x in first.tolist()]
        # 空欄は自動名、重複は一意化
        new_cols = _make_unique([c if c else "" for c in new_cols_raw])
        g = df.iloc[1:].copy()
        g.columns = new_cols
        g = _flatten_columns(g)  # 念のため再度クリーン＆一意化
        return g, 1
    else:
        return _flatten_columns(df), 0



def _read_excel_heads_fixed_header(path: Path, n: int, all_sheets: bool, header_row_index: int = 2) -> List[Tuple[str, pd.DataFrame]]:
    """
    プレビュー用（福島専用）。3行目(index=2)を一旦ヘッダーにして読む。
    必要なら 0 行目を本当の見出しとして昇格。
    """
    out: List[Tuple[str, pd.DataFrame]] = []
    try:
        xls = pd.ExcelFile(path)
        ...
        for s in targets:
            try:
                df = pd.read_excel(path, sheet_name=s, header=header_row_index, dtype=str)
                df, _ = _maybe_promote_first_row_to_header(df)  # ★昇格
...
def _read_excel_all_fixed_header(path: Path, header_row_index: int = 2) -> List[Tuple[str, pd.DataFrame, int, int]]:
    """
    照合用（福島専用）。
    返り値: [(sheet_name, df, header_idx, data_offset)]
      header_idx   … 0 始まりのヘッダー行（固定で 2）
      data_offset  … 0 or 1 （0 行目をヘッダー昇格したら 1）
    """
    out: List[Tuple[str, pd.DataFrame, int, int]] = []
    try:
        xls = pd.ExcelFile(path)
        for s in xls.sheet_names:
            try:
                df = pd.read_excel(path, sheet_name=s, header=header_row_index, dtype=str)
                df, data_offset = _maybe_promote_first_row_to_header(df)  # ★昇格
                out.append((s, df, header_row_index, data_offset))
            except Exception as e:
                st.warning(f"Excelシート読み込み失敗: {path.name} [{s}] ({e})")
    except Exception as e:
        st.warning(f"Excel読み込み失敗: {path.name} ({e})")
...
def _add_src_info(
    df: pd.DataFrame, *, file: Path, sheet: Optional[str] = None, header_idx: int = 0, data_offset: int = 0
) -> pd.DataFrame:
    """
    Excelの行番号を _src_row に付与。
    既定: 最初のデータ行 = header_idx + 2（Excel表記）
    0 行目をヘッダー昇格した場合は data_offset=1 → 最初のデータ行 = header_idx + 2 + 1
    """
    g = df.copy()
    g["_src_file"] = file.name
    if sheet is not None:
        g["_src_sheet"] = sheet
    g["_src_row"] = g.index + header_idx + 2 + data_offset
    return g

# ---- 列名クリーニング & 一意化 ----
def _clean_header_cell(x) -> str:
    s = "" if x is None else str(x)
    s = s.replace("\n", " ").replace("\u3000", " ").strip()  # 改行・全角スペース→半角空白
    # 全角英数→半角
...
def _make_unique(names: list[str]) -> list[str]:
    ...
    for n in names:
        ...
        if base not in seen:
            ...
            seen[base] += 1
            out.append(f"{base}.{seen[base]}")  # 例: 和名, 和名.1, 和名.2 ...
    return out
...
for folder, label in FOLDERS:
    ...
    for f in files:
        ...
        if ext in CSV_EXTS:
            ...
            if folder == "fukushima":
                heads = _read_excel_heads_fixed_header(f, n_rows, show_sheet_all, header_row_index=2)
            else:
                heads = _read_excel_heads(f, n_rows, show_sheet_all)  # 従来関数（他県・他ファイル用）
            if not heads:
                st.error("読み込みに失敗しました。")
            else:
...
# 便利：列名のゆらぎを吸収して欲しい列を取り出すヘルパ（メタ列はパススルー）
def pick_cols(
    df: pd.DataFrame,
    wanted: Dict[str, List[str]],
    passthrough_cols: List[str] = ["_src_file", "_src_sheet", "_src_row"]
) -> pd.DataFrame:
    """
    wanted = {"dst_col": ["候補1", "候補2", ...], ...}
    指定された列を新しいDataFrameで返す。passthrough_cols が存在すれば併せて含める。
    """
    cols = {}
    # 正規化（全角空白→半角、前後空白除去）
    def _norm(s: str) -> str:
...
def _read_excel_all(path: Path) -> List[Tuple[str, pd.DataFrame]]:
    """
    Excelの全シートを読み込み、(sheet_name, df) のリストを返す（照合用）
    """
    out: List[Tuple[str, pd.DataFrame]] = []
    try:
        xls = pd.ExcelFile(path)
...
def _add_src_info(df: pd.DataFrame, *, file: Path, sheet: Optional[str] = None) -> pd.DataFrame:
    """
    読み込んだDFに、ファイル名・シート名・行番号（ヘッダ1行を1と見なして +2）を付与
    """
    g = df.copy()
    g["_src_file"] = file.name
    if sheet is not None:
        g["_src_sheet"] = sheet
    # pandasのindexは0始まり。ヘッダが1行あると見なして+2（人間基準の行番号）
    g["_src_row"] = g.index + 2
    return g
...
if prec_dir.exists():
    for p in sorted(prec_dir.iterdir()):
        if p.suffix.lower() in {".xlsx", ".xls"} and p.is_file():
            for sheet_name, df in _read_excel_all(p):
...
if fuku_dir.exists():
    for p in sorted(fuku_dir.iterdir()):
        if p.suffix.lower() in {".xlsx", ".xls"} and p.is_file():
            for sheet_name, df, header_idx, data_offset in _read_excel_all_fixed_header(p, header_row_index=2):
                df = _add_src_info(df, file=p, sheet=sheet_name, header_idx=header_idx, data_offset=data_offset)
                sub = pick_cols(
                    df,
                    {
                        "福島県和名": ["和名"],
...
# 参照用辞書（キー = 正規化した和名）: カテゴリ + 出所（ファイル/シート/行）
moe_map: Dict[str, Dict[str, Optional[str]]] = {}
if not moe_df.empty:
    for _, r in moe_df.dropna(subset=["環境省和名"]).iterrows():
        key = _norm_name(r["環境省和名"])
        moe_map[key] = {
            "cat":  r.get("環境省カテゴリー"),
            "file": r.get("_src_file"),
            "row":  r.get("_src_row"),
            # CSVなので sheet は無し
        }

fuku_map: Dict[str, Dict[str, Optional[str]]] = {}
if not fuku_df.empty:
    for _, r in fuku_df.dropna(subset=["福島県和名"]).iterrows():
        key = _norm_name(r["福島県和名"])
        fuku_map[key] = {
            "cat":   r.get("福島県カテゴリー"),
            "file":  r.get("_src_file"),
            "sheet": r.get("_src_sheet"),
            "row":   r.get("_src_row"),
        }
...
for _, r in prec_df.iterrows():
    ...
    moe_hit  = moe_map.get(key)
    fuku_hit = fuku_map.get(key)

    if (moe_hit is not None) or (fuku_hit is not None):
        rows.append(
            {
                "prec種名": name,
