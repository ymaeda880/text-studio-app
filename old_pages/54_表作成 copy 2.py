...
def _build_html_table(rows: List[List[str]], *,
    ...
    td_css  = td_base + ("border:1px solid #000;" if (inner_h or inner_v) else "border:0;")
    zebra_alt="#F7F9FC"
    html=[f'<table style="{border_css}{outer_css};width:100%;">']
    if col_width_pct:
        html.append("<colgroup>")
        for p in col_width_pct: html.append(f'<col style="width:{p}%">')
        html.append("</colgroup>")
    ...
    for c in range(n_cols):
        ...
        html.append(
            ...
            f'font-family:{font_name};font-size:{header_size}pt;{bold_css}text-align:left;">{rows[0][c]}</th>'
        )
    html.append("</tr></thead><tbody>")
    for r in range(1,n_rows):
        row_bg = (zebra and r%2==1 and not body_bg) and zebra_alt or (body_bg or "transparent")
        html.append(f'<tr style="background:{row_bg};">')
        for c in range(n_cols):
            html.append(
                f'<td style="{td_css}font-family:{font_name};font-size:{body_size}pt;color:{body_fg};text-align:left;">{rows[r][c]}</td>'
            )
        html.append("</tr>")
    html.append("</tbody></table>")
    if note_text and note_text.strip():
        esc=(note_text.strip().replace("&","&amp;").replace("<","&lt;").replace(">","&gt;")).replace("\n","<br/>")
        html.append(f'<div style="margin-top:8px;font-family:{font_name};font-size:{body_size}pt;color:#444;">{esc}</div>')
    return "\n".join(html)
