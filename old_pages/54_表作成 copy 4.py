...
# ====== 連続同一セルの結合（Word/HTML 共用ロジック） ======
def _compute_spans(rows: List[List[str]], merge_v: bool, merge_h: bool, ignore_header: bool = True):
    """
    rowspan/colspan と skip 情報を返す。
    まず縦方向の連続同一値をまとめ（上端セルが rowspan>1 を持つ）、
    その後、横方向の連続同一値を各行ごとにまとめる（左端セルが colspan>1 を持つ）。
    空白やNoneは結合しない。
    """
    if not rows:
        return []
    ...
    if merge_v:
        for c in range(C):
            ...
            while r < R:
                ...
                while end < R and _norm(rows[end][c]) == val and not spans[end][c]["skip"]:
                    end += 1
                run_len = end - r
                if run_len > 1:
    ...
    if merge_h:
        for r in range(start_r, R):
            ...
            while c < C:
                ...
                while end < C and (not spans[r][end]["skip"]) and _norm(rows[r][end]) == val:
                    end += 1
                run_len = end - c
                if run_len > 1:
                    spans[r][c]["colspan"] = run_len
...
with st.sidebar:
    ...
    merge_h = st.checkbox("横方向に同一値を結合", value=False)
...
with c1:
    preset = st.radio("プリセット", list(PRESETS.keys()), index=0)
with c2:
...
if make_btn:
    ...
    # 罫線は最後に（結合後）— 安全版（内部線なし）
    _apply_table_inner_borders_after_merge(table, spans, vertical=inner_v, horizontal=inner_h, val="single", sz=6, color="000000")
    if outer:
