# -*- coding: utf-8 -*-
# pages/24_図表チェック.py（改良版：強調表示・excerpt付き）
# 目的：
#   PDFテキストから「図/表/図表」の
#   ① 行単独の見出し（キャプション）
#   ② 本文中の参照（行途中の言及）
# を “50_正規表現テスト” と同じ判定ロジック（1)行頭/2)直後の助詞/3)句点）で分離抽出。
# さらに各PDFページの「頁ラベル」も抽出し、page_label列として併記。
# （行抽出をマッチを含む1行に変更、強調⟪…⟫とexcerptを付与）
...
if df_captions.empty or "図表番号" not in df_captions.columns:
    ...
    # --- 1) 重複チェック（同じ番号の見出しが複数存在） ---
    #   キー：図表キー（= 種別 + 正規化番号）。同じキーが2個以上なら重複。
    # --- 1) 重複チェック（続き判定込み） ---
    #   同じ図表キーが2件以上あるとき：
    #   ・タイトル本体が同じで (1/2),(2/2)... の分割表記のみが違い
    #   ・pdf頁が連続（差が全て1）
    #   なら「続き」とみなし、重複にカウントしない

    def _title_base(s: str) -> str:
        """(1/2) 等の分割表記を除去し、空白類を正規化したタイトル本体を返す"""
        if s is None:
            return ""
        t = str(s)
        # 全角括弧も対象にして (1/2) / （1/2） / ( 1 / 2 ) を除去
    ...
    def _part_tuple(s: str) -> Tuple[Optional[int], Optional[int]]:
        """タイトル末尾から (i/n) を取り出す（なければ (None, None)）"""
        if not s:
            return (None, None)
        m = re.search(r"[\(（]\s*(\d+)\s*/\s*(\d+)\s*[\)）]\s*$", str(s))
        if not m:
            return (None, None)
    ...
    def _is_continuation_group(g: pd.DataFrame) -> bool:
        """
        同じ図表キーのグループ g について、続きのページ群かどうかを判定
        条件：
        1) タイトル本体（_title_base）が全レコード同一
        2) pdf_page が昇順で連続（差の最大が 1）
        3) (i/n) が付いている場合は i が 1,2,3... と増加かつ n が一定（緩和：i/n が無くても OK）
        """
        if g.shape[0] <= 1:
            return False
        ...
        if not diffs or max(diffs) != 1:   # すべて差が1（連続）
            return False
        # (i/n) チェック（あれば厳密、無ければスキップ）
        parts = [ _part_tuple(t) for t in g2["見出しタイトル"].fillna("") ]
        if any(p[0] is None or p[1] is None for p in parts):
            return True  # 分割表記なしでも連続ページなら続きとみなす
        nums = [p[0] for p in parts]     # i
        totals = {p[1] for p in parts}   # n
        if len(totals) != 1:
            return False
        # 例：1,2,3,... と単調増加
        return nums == list(range(min(nums), min(nums) + len(nums)))

    # グループ化して「続き」か「真の重複」かを分類
    cont_rows = []   # 続きのページ
    dup_rows  = []   # 真の重複
    for k, g in df_captions.groupby("図表キー"):
        if len(g) <= 1:
            continue
        if _is_continuation_group(g):
            g2 = g.sort_values("pdf_page")
            cont_rows.append({
                "図表キー": k,
                "図表タイトル（本体）": _title_base(g2["見出しタイトル"].iloc[0]),
                "pdf頁一覧": ",".join(map(lambda x: str(int(x)), g2["pdf_page"].tolist())),
                "頁ラベル一覧": ",".join([str(x) for x in g2["page_label"].fillna("").tolist()]),
                "備考": "（続きのページとみなす）"
            })
        else:
            # 本当に同番号の見出しが複数存在（重複疑い）
            g2 = g.sort_values("pdf_page")
            dup_rows.append({
                "図表キー": k,
    ...
    # --- 2) 連番チェック：飛び・開始が1でない ---
    #  仕組み：
    #   - 図表番号（例：図3-2, 表4.1-3 など）から
    #     「系列プレフィックス（最後の“-”より前）」と「末尾インデックス（整数）」に分割
    #   - 種別（図/表/図表）× プレフィックスごとに、末尾インデックスの連続性を検査
    import math

    def _numeric_core_from_key(key: str) -> str:
        # 図表キー = 種別 + 正規化番号（例: "図3-2", "表4.1-3"）
        # 先頭の種別（図/表/図表）を除去
    ...
    def _series_and_index(key: str) -> Tuple[str, Optional[int], str]:
        """
        return:
          series: 連番系列（= 最後の '-' より前。なければ ''）
          idx   : 末尾の整数インデックス（取れなければ None）
          kind  : 図/表/図表 の種別（見やすさ用）
        """
        m = re.match(r"^(図表|図|表)", key)
        kind = m.group(1) if m else ""
        num_part = _numeric_core_from_key(key)  # 例: "3", "3-2", "4.1-3"
        # 系列/末尾
        if "-" in num_part:
    ...
    for (kind, series), items in series_map.items():
        items_sorted = sorted(items, key=lambda x: (x[0], x[2]))  # idx -> pdf頁
...
cap_idx = _index_pages_by_key(df_captions)  # 見出し側：図表キー -> ページ配列
ref_idx = _index_pages_by_key(df_refs)      # 参照側：図表キー -> ページ配列
...
# ===== 画面表示用：引用されている見出し（見出し あり ＆ 参照 あり）=====
def _ref_aggregate_for_view(df_refs: pd.DataFrame):
    """図表キーごとに参照の頁ラベル/ pdf頁 / 行テキスト(通常/強調)を集約して返す"""
    ...
    for df, name in [
        ...
        if not df.empty:
            ...
            if "page_label" in df2.columns:
                df2["page_label"] = df2["page_label"].map(_protect_for_excel)
            buf = io.StringIO()
...
# =========================
# 🔽 突き合わせ結果の XLSX ダウンロード（参照頁ラベル・参照pdf頁 追加）
# =========================
from io import BytesIO

def _caption_info_first_by_key(df: pd.DataFrame) -> dict:
    """図表キー -> 最初のキャプション行（pdf_page最小）の情報"""
    info = {}
...
def _pages_str(pages: List[int]) -> str:
    return ",".join(str(int(p)) for p in pages) if pages else ""

def _protect_for_excel(x: object) -> object:
    # "1-2" などの頁ラベルが日付化されるのを防止
    if isinstance(x, str) and re.match(r"^\s*\d{1,2}\s*[-−ー－―]\s*\d{1,2}\s*$", x.strip()):
        return f'="{x.strip()}"'
    return x
...
# ========= Excel 出力 =========
with st.sidebar:
    st.markdown("### 🔗 突き合わせ結果のダウンロード（XLSX）")
    output = BytesIO()
