...
if do_search and queries:
    ...
    # --- 未ヒット語（該当なし）を作成 ---
    # ここでは「完全一致前提」で“和名”と照合（部分一致モードでは未ヒット判定は行わない）
    ...
    if matched_df.empty and missing_df.empty:
        ...
        # --- 同じ和名が連続したら2行目以降を空白化（未ヒットは別和名のため影響なし） ---
...
with st.expander("ℹ️ 備考", expanded=False):
    st.markdown("""
- **並べ替え**：入力した検索語の順番に結果を並べています。未ヒット語は末尾に入力順でまとめています。  
...
- **該当なし**は完全一致モード時にのみ判定します（部分一致モードでは未判定）。
    """)
