...
領域内に複数のコンテンツを記述した場合は，
SlideTexへ記述した順番に，上から下へ配置します．
...
最初に，`layout`によって各領域の位置と大きさを決めます．
...
では，`left`と`right`の2つの領域を作成します．

`ratio`が指定されている場合は，
その比率を使って各領域の幅または高さを決めます．

---

###### ② 領域の内側に余白を取る

各領域の外枠いっぱいにコンテンツを配置するのではなく，
領域の内側に一定の余白を取ってからコンテンツを配置します．
...
したがって，最初のコンテンツは，
領域の上端ではなく，上端から0.22インチ下げた位置から始まります．
...
領域内の各コンテンツには，
コンテンツの種類と内容に応じて縦方向の高さを割り当てます．

現在，主な文字系コンテンツの基準高さは次のように計算します．
...
現在の位置決めでは，
領域内のすべてのコンテンツが`text`や`itemize`などの
固定高さを持つコンテンツであり，
それらを配置しても領域内に高さが余る場合は，
余った高さを各コンテンツへ均等に追加します．
...
の2つだけが領域内にあり，大きな空き領域が残る場合は，
余った高さを2つのコンテンツへ均等に配分します．

そのため，実際に表示されている文字の高さよりも，
コンテンツへ割り当てられた領域が大きくなり，
次のコンテンツとの間隔が広く見える場合があります．
...
この場合，文字そのものに大きな余白が設定されているのではなく，
前のコンテンツへ割り当てられた領域の高さによって，
次のコンテンツの開始位置が下へ移動しています．

---

###### 領域内に高さが不足する場合

コンテンツの基準高さの合計が領域内に収まらない場合は，
領域内へ収めるために各コンテンツの割り当て高さを縮小します．

ただし，極端に小さくならないように，
コンテンツには最小高さが設定されています．

現在の標準値は次のとおりです．

```text
通常コンテンツの最小高さ    0.34インチ
可変コンテンツの最小高さ    0.80インチ
```

この位置決めは，各領域ごとに独立して行います．

例えば`two_column`では，
`left`と`right`についてそれぞれ個別に，
領域内のコンテンツの高さと位置を計算します．
