...
def _compute_spans_markers(
    ...
    *,
    ...
    header_rows: int = 1,     # 先頭から何行をヘッダー扱いにするか（現在ロジックでは未使用だが将来拡張用）
...
def _merge_docx_by_spans(
    ...
    header_same: bool,
    body_bg_on: bool,
    header_rows: int = 1,  # ★ ヘッダー行数（0 も許可）
):
    R, C = len(rows), len(rows[0])
    ...
    # ★ ここを「0 も許す」ように変更
    header_rows = max(0, min(header_rows, R))  # 安全ガード（0〜R）
    ...
    for r in range(R):
        for c in range(C):
            ...
            is_header = (r < header_rows)  # ★ 0 のときは常に False
...
def _build_html_table_with_spans(
    ...
    *,
    ...
    # --- thead（先頭 header_rows 行）---
    if header_rows > 0:        # ★ ここを条件付きに
        html.append("<thead>")
