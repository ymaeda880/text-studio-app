    ...
    # },
     ...
     "解析文書校正（通常校正）": {
        ...
        "analyze_inst": (
            ...
            "## 助詞（てにおは）、主述一致、冗長、重複、語順、誤字脱字、用語誤用、文体の不統一に特に注意。\n"
            "（表についての指示）\n"
            "## <ここから表>と<ここまで表>の間はwordの「表」を整形したものです\n"
            "<ここから表>と<ここまで表>の表のブロックはあくまでも，あなた（AI）が理解しやすいようにプログラムの内部で使用しているものですので，校正方針で指摘するときは「表ブロック」などという言葉は使用せず，単に「表」と表現してください．"
            "- <ここから表>のすぐ下に「表番号」と「表タイトル」が,通常はあります．"
            "（例：表2.1-1　学生の成績)（この例では「表番号」は「2.1-1」,「表タイトル」は「学生の成績」です\n"
            "- 「表番号」の前には通常「表」という言葉があります．\n"
            "- <ここから表>のすぐ下に「表番号」と「表タイトル」が,ないこともあります．その場合は誤りとして指摘しないでください．"
            "- <ここから表>と「表タイトル」の間にある文は，表の説明書きです．\n"
            "- []が表の1つの行を表しています．\n"
            "- []の中に，表の各行の項目（セル）が「,」で区切られて入っています\n"
            "- 表の中の最初の[]は列ラベルであることが多いです\n"
            "- 元々のセルが縦に結合されているときは，「同一の単語」が縦に並びます．\n"
            "- 元々のセルが横に結合されているときは，右のセルに「<同左>」という文字が入ります．\n"
            "- <ここまで表>の下に「出典」で始まる行は，上の表の出典です．\n"
            "- <ここまで表>の下に「備考」で始まる行は，上の表の備考です．\n"
            ...
            "- （重要）表の各行の区切り記号や空白の入り方を校正の対象としないでください．\n"
            "- （重要）表のタイトルや内容と，表を参照している本文の記述が矛盾していないかどうか調べてください．\n"
             "## <ここから見出し>と<ここまで見出し>の間はwordの「見出し」を整形したものです\n"
             "## <ここから見出し>と<ここまで見出し>はあなた（AI）が理解しやすいように内部のプログラムで挿入しているものです．校正方針には，それらのコードそのものに関する指摘はしないでください．\n"
            "（その他の指示）\n"
            "## wordの文書を一度変換したものを入力しています．従って，数式の累乗などは崩れていますので，間違いとして指摘しないでください．"
            "（例：本来「H^2」であったものは「H２」などとなっています．）\n"
            "<ここから表>と<ここまで表>が対で見つからない場合でも，そのことを誤りとして指摘しないでください．"
            "<ここから表>と<ここまで表>が対で見つからない場合でも，表の範囲を推測して表として扱ってください．"
            "AIへの入力トークン数の上限があるため，入力した文章が途中で終わっている場合，途中から始まっている場合があります．そのことを校正方針で指摘しないでください．"
            "「出典」の記載がある場合に，出典がその行ではなく，次の行から記載されていることもあります．"
            "今回の校正は「通常校正」として処理してください\n"
        ),
    },
}
